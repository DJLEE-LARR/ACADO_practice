...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) 
 { 
    ...
    DifferentialState alphadot;
    DifferentialState D;
    Control F;
    DMatrix acadodata_M1;
    acadodata_M1.read( "inverted_pendulum_data_acadodata_M1.txt" );
    DifferentialEquation acadodata_f1(0, 10);
    acadodata_f1 << dot(x) == xdot;
    acadodata_f1 << dot(alpha) == alphadot;
    acadodata_f1 << dot(xdot) == (-2.09999999999999992228e-01*9.81000000000000049738e+00*cos(alpha)*sin(alpha)+6.40499999999999958256e-02*alphadot*alphadot*sin(alpha)+F)/(2.09999999999999992228e-01*sin(alpha)*sin(alpha)+4.55000000000000015543e-01);
    acadodata_f1 << dot(alphadot) == (-(-2.09999999999999992228e-01*9.81000000000000049738e+00*cos(alpha)*sin(alpha)+6.40499999999999958256e-02*alphadot*alphadot*sin(alpha)+F)/(2.09999999999999992228e-01*sin(alpha)*sin(alpha)+4.55000000000000015543e-01)*cos(alpha)+9.81000000000000049738e+00*sin(alpha))*3.27868852459016402179e+00;
    acadodata_f1 << dot(D) == 5.00000000000000000000e-01*F*F;

    OCP ocp1(0, 3, 50);
    ocp1.minimizeMayerTerm(D);
    ocp1.subjectTo(acadodata_f1);
    ocp1.subjectTo(AT_START, x == 0.00000000000000000000e+00);
    ocp1.subjectTo(AT_START, alpha == (-3.14159265358979311600e+00));
    ocp1.subjectTo(AT_START, xdot == 0.00000000000000000000e+00);
    ocp1.subjectTo(AT_START, alphadot == 0.00000000000000000000e+00);
    ocp1.subjectTo(AT_START, D == 0.00000000000000000000e+00);
    ocp1.subjectTo(AT_END, alpha == 0.00000000000000000000e+00);
    ocp1.subjectTo(AT_END, xdot == 0.00000000000000000000e+00);
    ocp1.subjectTo(AT_END, alphadot == 0.00000000000000000000e+00);
    ocp1.subjectTo((-1.00000000000000000000e+02) <= F <= 1.00000000000000000000e+02);
